...
char showDetail (Details * Detail, char medium)
{
	...
	if (Detail != NULL)
	{
		...
		//ToDo Workaround wenn in Verlag und Regisseur nichst drin steht...
		// Kann später nicht mehr passieren
		if(Detail->regisseur[0] == -51)
		{
			Detail->regisseur[0] = '\0';
		}
		if(Detail->verlag[0] == -51)
		{
			Detail->verlag[0] = '\0';
		}
		//ToDo Workaround wenn in Verlag und Regisseur nichst drin steht...
		...
		ISBN = Detail->isbn;
		printf("ISBN:  %i-", abs(ISBN/10000000000));
